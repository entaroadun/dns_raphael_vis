    ...
    function block() {
		...
		min_val = users[i][1];
            ...
            for (var i = 0, ii = users.length; i < ii; i++) {
                ...
                // -- original formula  -->  p.b.unshift([x, h += Math.max(Math.round(Math.log(users[i][1]) * 5), 1)]);
                p.b.unshift([x, h += Math.round(((users[i][1]-min_val)/(max_val-min_val))*(49-Math.log(users[i][1])))+Math.round(Math.log(users[i][1])+1)]);
        ...
        for (var i in pathes) {
            ...
            (function (i) {
                pathes[i].p.mouseover(function () {
                    ...
                    pathes[i].p.toFront();
                    labels[i].toFront();
                    usrnm2.innerHTML = json.authors[i].n + " <em>(min " + json.authors[i].c + ", average " + json.authors[i].a + ", max " + json.authors[i].d + ")</em>";
                    lgnd2.style.backgroundColor = pathes[i].p.attr("fill");
                    nmhldr2.className = "";
                    plchldr.className = "hidden";
		    $('#chart').css('margin-top','32px');
                });
            })(i);
        }
    }
